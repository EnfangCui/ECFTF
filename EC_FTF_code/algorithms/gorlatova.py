class Gorlatova():
    ...
    def __init__(self, slots_per_cycle, bmin):
    ...
    def check_validity(self, crt_alloc, cycle_eh, B0, test_i, i):
        ...
        """
        B_crt = B0
        for s in range(len(cycle_eh)): # The final cycle might be shorter
            if s != i:
                B_crt = B_crt + cycle_eh[s] - crt_alloc[s]
            else:
                B_crt = B_crt + cycle_eh[s] - test_i
            if B_crt < self.bmin: # or B_crt > ehct.bmax:
                #print "Error"
                return False
        if B_crt < B0:
            #print "Offset"
    ...
    def allocate(self, eh_pred, B0):
        ...
        self.alloc = [0 for i in range(len(eh_pred))]
